...
import type { Job, JobStore, JobMode } from "@/lib/types";
...
function SkeletonGrid() {
  return <>
    {Array.from({ length: 6 }).map((_, i) => (
      <div key={i} className="skeleton" style={{ animationDelay: `${i * 0.12}s`, height: `${260 + (i % 3) * 20}px` }} />
    ))}
  </>;
}
...
export default function Dashboard() {
  ...
  const bestScore = useMemo(() => modeJobs.length ? Math.max(...modeJobs.map(j => j.totalScore)) : 0, [modeJobs]);
  ...
  const filtered = useMemo(() => {
    ...
    return modeJobs
      ...
      .filter(j => !q || j.title.toLowerCase().includes(q) || j.company.toLowerCase().includes(q) || j.matchedSkills.some(s => s.toLowerCase().includes(q)))
      .sort((a, b) =>
    ...
    <div className="app-shell">
      ...
      <header className="app-header">
        <div className="header-inner">
          <div className="brand">
            <div className="radar-dot"><span className="radar-pulse" /></div>
            <div>
              <h1 className="brand-title">JOB RADAR</h1>
              <p className="brand-sub">Frontend roles · Direct from company career pages · No aggregators</p>
            </div>
          </div>
          <div className="header-actions">
            {store?.lastUpdated && <span className="last-updated">Updated {relativeTime(store.lastUpdated)}</span>}
            ...
            <button className={`btn-run ${running ? "running" : ""}`} onClick={runScan} disabled={running}>
      ...
      <div className="filters-bar">
        ...
        <div className="filter-score-wrap">
          ...
          <input type="range" className="filter-range" min={0} max={100} value={minScore} onChange={e => setMinScore(Number(e.target.value))} />
        </div>
